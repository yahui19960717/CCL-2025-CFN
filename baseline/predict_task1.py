...
def test(model, test_loader, file_out=None):
    # model.eval()
    model.train()
    all_dataset = []
...
if __name__ == '__main__':
    ...
    file_out = 'dataset/seed77/A_task1_test.json'
    # args.gamma = 4.0
    ...
    # state = torch.load("saves/model_task1_best_robert_large.bin", map_location='cpu')
    # state = torch.load("saves/model_task1_focal_loss_gamma4.0_alpha_None.bin", map_location='cpu')
    # state = torch.load("saves/model_task1_best_roberta_large_word_embedding_ws.bin", map_location='cpu')
