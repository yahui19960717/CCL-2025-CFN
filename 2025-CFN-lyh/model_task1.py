...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        if self.config.use_ws:
            self.ws_embedding = nn.Embedding(5, 16) #16
            input_size += 16
    ...
    def forward(self, input_ids=None, attention_mask=None, target=None, labels=None, pos_labels=None, dep_labels=None, device=None, pos=None, dep=None, ws_label=None, word_indices=None, for_test=False):
        ...
        # outputs:(batch_size, seq_len, ent_type_size, seq_len)
        outputs = torch.stack(outputs, dim=-2)
        # 对 token 表示进行线性变换生成 query 和 key。
        ...
        # 对所有 token 位置对做点积计算 logits，构造 logits 张量：每对 token 都建模是否属于某一 frame
        logits = torch.einsum('bmhd,bnhd->bhmn', qw, kw)
        logits = logits / self.inner_dim ** 0.5
        # logits:(batch_size, ent_type_size, seq_len, seq_len) # 选出每个目标词对应的labels得分，最后得到一批目标词的框架得分
        # token_logits:(batch_size, ent_type_size)
