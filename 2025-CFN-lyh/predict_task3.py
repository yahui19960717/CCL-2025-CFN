...
def test(model, val_loader, file_out=None,type_eval="eval", device=None):
    if type_eval=="eval":
        model.eval()
    else:
        model.train()
...
def main_function(seed, file_out, type_eval="eval"):
    ...
    test_dataset = Dataset("./dataset/cfn_ws/cfn-test-B-ws.json",
                            ...
                            "./dataset/submit/B_task2_test.json",
                            tokenizer,
                            ws=args.use_ws,
                            use_embedding=args.use_embedding,
                            embed_file="./dataset/cc.zh.300.cfnAB.vec")
    args.gamma = 1.0
    ...
    state = torch.load("saves/model_task3_best_roberta_large_targetwenofreeze_focal_loss_gamma1.0_ws.bin", map_location='cpu')
    ...
    test(model, test_loader, file_out, type_eval="eval", device=device)
...
if __name__ == '__main__':
    ...
    main_function(seed=seed, file_out='dataset/seed1/B_task3_test.json')
    time.sleep(60)
    ...
    print(f"正在运行seed = {seed}:")
    main_function(seed=seed, file_out='dataset/seed77/B_task3_test.json', type_eval="train")

    # seed777
    seed=777
    os.environ['CUDA_VISIBLE_DEVICES'] = '5'
    print(f"正在运行seed = {seed}:")
    main_function(seed=seed, file_out='dataset/seed777/B_task3_test.json', type_eval="train")
