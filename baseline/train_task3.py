...
def get_model_input(data, device=None, ws=False, use_embedding=False):
    ...
    for d in data:
        ...
        if ws:
            ws_label.append(pad(d[5], max_len, 0))
        if use_embedding:
    ...
    return input_ids, attention_mask, target, labels, sentence_id, ws_label, word_indices
...
def eval(model, val_loader):
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(val_loader), total=len(val_loader), desc='eval'):
            input_ids, attention_mask, target, labels, sentence_id, ws_label, word_indices = batch
...
if __name__ == '__main__':
    ...
    # exp_name = f'saves/model_task3_focal_loss_gamma2.0_alpha_None.bin'
    # exp_name = f'saves/model_task3_gamma3.0_alpha_None.bin'
    # exp_name = f'saves/model_task3_best_roberta_large_targetembedding_focal_loss_gamma1.0_nows.bin'
    exp_name = f'saves/model_task3_best_roberta_large_targetwenofreeze_focal_loss_gamma1.0_ws.bin'
